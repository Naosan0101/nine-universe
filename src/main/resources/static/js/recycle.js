(function () {
	...
	const PACK_IDS_STANDARD_3 = ['OT', 'CS'];
	...
	function openCardDetailModal(c, navOpts) {
		...
		function goKingMakerInkKingRecycle() {
			if (!FossilUi) return;
			const compRaw = FossilUi.parseCompanionJson(c.companionDetailJson);
		...
		function goLuciferFallenRecycle() {
			if (!FossilUi) return;
			const compRaw = FossilUi.parseCompanionJson(c.companionDetailJson);
		...
		if (FossilUi) {
			FossilUi.appendAbilityBlocksToModal(modalAbility, blocks, c.companionDetailJson, goCompanionRecycle);
